...
async function fetchCombinedBlobURL(urls: readonly string[], mimeType: string, signal: AbortSignal): Promise<string> {
...
async function withTimeout<T>(timeoutMs: number, operation: (signal: AbortSignal) => Promise<T>): Promise<T> {
  ...
  const timeoutId = window.setTimeout(() => controller.abort(new DOMException(`Timed out after ${timeoutMs}ms`, 'AbortError')), timeoutMs);
  try {
...
async function loadFFmpegCoreFromSource(ffmpeg: FFmpeg, candidate: CoreCandidate) {
  const { baseURL, multithread } = candidate;
  let coreURL: string | null = null;
  let wasmURL: string | null = null;
  let workerURL: string | null = null;
  try {
    coreURL = await withTimeout(FFMPEG_ASSET_TIMEOUT_MS, (signal) => fetchBlobURL(`${baseURL}/ffmpeg-core.js`, 'text/javascript', signal));
    wasmURL = await withTimeout(FFMPEG_ASSET_TIMEOUT_MS, (signal) => baseURL === LOCAL_FFMPEG_CORE_BASE_URL
    ...
    if (multithread) {
      workerURL = await withTimeout(FFMPEG_ASSET_TIMEOUT_MS, (signal) => fetchBlobURL(`${baseURL}/ffmpeg-core.worker.js`, 'text/javascript', signal));
    }
  ...
  loadPromise = (async () => {
    ...
    for (const candidate of getPreferredFFmpegCoreCandidates()) {
      ...
      try {
        ...
        console.log(`[FFmpeg] Loaded ${candidate.multithread ? 'multi-thread' : 'single-thread'} core from:`, candidate.baseURL);
...
export type FFmpegStage = 'downloading' | 'converting' | 'reading-video' | 'reading-audio' | 'merging';
...
export interface ProgressInfo { loaded?: number; total?: number; }
...
async function mountWorkerFile(ffmpeg: FFmpeg, file: File, mountPoint: string): Promise<{ path: string; cleanup: () => Promise<void> }> {
...
async function readLocalFile<TStage extends FFmpegStage>(file: File, stage: TStage, signal?: AbortSignal, onProgress?: (progress: number, stage: TStage, info?: ProgressInfo) => void, progressStart = 0, progressEnd = 100): Promise<Uint8Array> {
  return new Promise((resolve, reject) => {
    ...
    const rejectOnce = (error: Error) => { if (settled) return; settled = true; cleanupAbort(); reject(error); };
    const resolveOnce = (data: Uint8Array) => { if (settled) return; settled = true; cleanupAbort(); resolve(data); };
    const handleAbort = () => {
    ...
    reader.onload = (e) => {
      ...
      } else rejectOnce(new Error('Failed to read file as ArrayBuffer'));
    };
...
async function downloadVideoData(videoUrl: string, signal?: AbortSignal, onProgress?: (progress: number, stage: ExtractStage, info?: ProgressInfo) => void): Promise<Uint8Array> {
...
export async function extractAudioFromVideo({ videoUrl, videoFile, signal, onProgress }: ExtractAudioOptions): Promise<Blob> {
  ...
  try {
    ...
    } else throw new Error('Either videoUrl or videoFile must be provided');
    const handleProgress = ({ progress }: { progress: number }) => onProgress?.(Math.round(progress * 100), 'converting');
    ffmpeg.on('progress', handleProgress);
    try {
      await ffmpeg.exec(['-i', inputPath, '-vn', '-acodec', 'libmp3lame', '-q:a', '2', 'output.mp3'], undefined, { signal });
    ...
    else await ffmpeg.deleteFile(inputPath).catch(() => undefined);
  }
}

export async function mergeVideoAudio({ videoFile, audioFile, signal, onProgress }: MergeVideoAudioOptions): Promise<Blob> {
  const ffmpeg = await getFFmpeg();
  ...
  try {
    ...
    const handleProgress = ({ progress }: { progress: number }) => onProgress?.(50 + Math.round(progress * 50), 'merging');
    ffmpeg.on('progress', handleProgress);
    try {
      await ffmpeg.exec(['-i', videoMount.path, '-i', audioMount.path, '-c:v', 'copy', '-c:a', 'aac', '-map', '0:v:0', '-map', '1:a:0', '-shortest', '-movflags', '+faststart', outputName], undefined, { signal });
