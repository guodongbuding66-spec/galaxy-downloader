...
export function normalizeNfoSidecarCapability(value: unknown): boolean {
  if (!value || typeof value !== 'object') return false
  const payload = value as Record<string, unknown>
  return payload.ok === true && payload.nfoSidecar === true
}

export async function getNfoSidecarCapability(
  ...
  for (const url of LOOPBACK_STATUS_URLS) {
    ...
    try {
      ...
      return normalizeNfoSidecarCapability(await response.json() as unknown)
    } catch {
      // Missing, older and offline engines fail closed. Try the alternate
      // loopback hostname before deciding that NFO sidecars are unavailable.
    } finally {
