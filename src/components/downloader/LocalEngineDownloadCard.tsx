...
const COPY: Record<string, Copy> = {
  zh: {
    ...
    intro: '遇到 YouTube、B站、小红书、快手、Dailymotion 等反爬或 IP 绑定限制时，直接调用你电脑上的 yt-dlp + FFmpeg。默认获取最佳画质与最佳音频，并在本机合并成最终文件。',
    cookieSource: '登录状态',
    noCookies: '不读取浏览器 Cookie',
    edge: '使用 Edge 登录状态',
    chrome: '使用 Chrome 登录状态',
    firefox: '使用 Firefox 登录状态',
    launch: '本机 yt-dlp 一键下载',
    install: '安装本地引擎',
  ...
  'zh-tw': {
    ...
    intro: '遇到反爬或 IP 綁定限制時，直接呼叫電腦上的 yt-dlp + FFmpeg，以最佳畫質與音訊在本機產生最終檔案。',
    cookieSource: '登入狀態',
    noCookies: '不讀取瀏覽器 Cookie',
    edge: '使用 Edge 登入狀態',
    chrome: '使用 Chrome 登入狀態',
    firefox: '使用 Firefox 登入狀態',
    launch: '本機 yt-dlp 一鍵下載',
    install: '安裝本地引擎',
  ...
  en: {
    ...
    intro: 'For anti-bot, login, or IP-bound platforms, run yt-dlp + FFmpeg on this computer. It downloads the best video/audio and builds the finished file locally.',
    ...
    launch: 'Download with local yt-dlp',
  ...
  ja: {
    ...
    intro: 'ログイン、Bot 対策、IP 制限があるサイトでは、この PC の yt-dlp + FFmpeg を使って最高品質を直接取得・結合します。',
    cookieSource: 'ログイン状態',
    noCookies: 'ブラウザー Cookie を使用しない',
    edge: 'Edge のログイン状態を使用',
    chrome: 'Chrome のログイン状態を使用',
    firefox: 'Firefox のログイン状態を使用',
    launch: 'ローカル yt-dlp でダウンロード',
  ...
  es: {
    ...
    intro: 'Para sitios con inicio de sesión, anti-bot o bloqueo por IP, usa yt-dlp + FFmpeg directamente en este equipo y genera aquí el archivo final.',
    ...
    launch: 'Descargar con yt-dlp local',
  ...
  ru: {
    ...
    launch: 'Скачать локально через yt-dlp',
...
export function LocalEngineDownloadCard({ result }: { result: ResultData }) {
  const pathname = usePathname();
  const copy = copyFor(pathname);
  const sourceUrl = scopedSourceUrl(result);
  const firstSubtitle = result.subtitles?.[0];
  ...
  const handleLaunch = () => {
    launchLocalDesktopEngine({
      ...
      audioQuality: 'best',
      includeAudio: true,
      includeSubtitle: Boolean(firstSubtitle),
      subtitleLanguage: firstSubtitle?.language || null,
      includeCover: Boolean(result.cover),
      browser,
      playlist: false,
    });
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-primary/20 bg-primary/[0.035] shadow-sm">
      <div className="space-y-4 p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Button type="button" className="min-h-11 font-semibold" onClick={handleLaunch}>
