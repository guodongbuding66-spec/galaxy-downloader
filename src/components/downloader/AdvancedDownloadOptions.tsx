...
const COPY: Record<string, Copy> = {
    zh: {
        title: '高级下载选项', quality: '视频清晰度',
        qualityHint: '按所选清晰度重新向解析后端请求媒体，不再复用可能偏低清晰度的临时 CDN 流。若平台没有该档位，后端应回退到最接近或最佳可用画质。',
        parserFormats: '解析器返回的可用格式', genericPresets: '通用画质预设', downloadVideo: '下载所选画质',
        audioQuality: '音频质量', downloadAudio: '下载所选音质', extras: '附加资源', cover: '下载封面',
        subtitles: '下载字幕', noSubtitles: '当前解析结果未返回可下载的字幕轨道', metadata: '下载媒体信息 JSON',
        copySource: '复制原始链接', copied: '原始链接已复制', best: '最佳可用',
    },
    'zh-tw': {
        title: '進階下載選項', quality: '影片畫質',
        qualityHint: '依所選畫質重新向解析後端請求媒體，不再重複使用可能偏低畫質的臨時 CDN 串流。若平台沒有該畫質，後端應回退至最接近或最佳可用畫質。',
        parserFormats: '解析器回傳的可用格式', genericPresets: '通用畫質預設', downloadVideo: '下載所選畫質',
        audioQuality: '音訊品質', downloadAudio: '下載所選音質', extras: '附加資源', cover: '下載封面',
        subtitles: '下載字幕', noSubtitles: '目前解析結果未回傳可下載的字幕軌道', metadata: '下載媒體資訊 JSON',
        copySource: '複製原始連結', copied: '原始連結已複製', best: '最佳可用',
    },
    en: {
        title: 'Advanced download options', quality: 'Video quality',
        ...
        parserFormats: 'Formats reported by parser', genericPresets: 'Generic quality presets', downloadVideo: 'Download selected quality',
        audioQuality: 'Audio quality', downloadAudio: 'Download selected audio', extras: 'Extra resources', cover: 'Download cover',
        subtitles: 'Download subtitles', noSubtitles: 'No downloadable subtitle tracks were returned by the parser', metadata: 'Download media info JSON',
        copySource: 'Copy source URL', copied: 'Source URL copied', best: 'Best available',
    },
    ja: {
        title: '詳細ダウンロード設定', quality: '動画画質',
        qualityHint: '低画質の一時 CDN URL を再利用せず、選択した画質で解析バックエンドへ再リクエストします。利用できない場合は最も近い／最良の画質へフォールバックします。',
        parserFormats: '解析結果の利用可能フォーマット', genericPresets: '共通画質プリセット', downloadVideo: '選択画質でダウンロード',
        audioQuality: '音声品質', downloadAudio: '選択音質でダウンロード', extras: '追加リソース', cover: 'サムネイルを保存',
        subtitles: '字幕を保存', noSubtitles: '解析結果にダウンロード可能な字幕トラックがありません', metadata: 'メディア情報 JSON を保存',
        copySource: '元 URL をコピー', copied: '元 URL をコピーしました', best: '最高品質',
    },
    es: {
        title: 'Opciones avanzadas de descarga', quality: 'Calidad de vídeo',
        ...
        parserFormats: 'Formatos informados por el analizador', genericPresets: 'Preajustes genéricos', downloadVideo: 'Descargar calidad elegida',
        audioQuality: 'Calidad de audio', downloadAudio: 'Descargar audio elegido', extras: 'Recursos adicionales', cover: 'Descargar portada',
        subtitles: 'Descargar subtítulos', noSubtitles: 'El analizador no devolvió pistas de subtítulos descargables', metadata: 'Descargar información JSON',
        copySource: 'Copiar URL original', copied: 'URL original copiada', best: 'Mejor disponible',
    },
    ru: {
        title: 'Расширенные параметры загрузки', quality: 'Качество видео',
        ...
        parserFormats: 'Форматы от парсера', genericPresets: 'Универсальные пресеты', downloadVideo: 'Скачать выбранное качество',
        audioQuality: 'Качество аудио', downloadAudio: 'Скачать выбранное аудио', extras: 'Дополнительные ресурсы', cover: 'Скачать обложку',
        subtitles: 'Скачать субтитры', noSubtitles: 'Парсер не вернул доступных дорожек субтитров', metadata: 'Скачать сведения JSON',
        copySource: 'Копировать исходную ссылку', copied: 'Исходная ссылка скопирована', best: 'Лучшее доступное',
    },
};

function resolveCopy(pathname: string): Copy {
...
export function AdvancedDownloadOptions({ result }: { result: ResultData }) {
    ...
    const persistVideoQuality = (value: string) => {
        ...
        try { window.localStorage.setItem('galaxy:video-quality', value); } catch { /* optional */ }
    };

    const persistAudioQuality = (value: string) => {
        setAudioQuality(value);
        try { window.localStorage.setItem('galaxy:audio-quality', value); } catch { /* optional */ }
    };
    ...
    const handleAudioDownload = () => {
        if (!sourceUrl) return;
        downloadFile(buildSourceMediaDownloadUrl({ sourceUrl, type: 'audio', quality: audioQuality }));
    };
    ...
    const handleSubtitleDownload = () => {
        const index = subtitles.findIndex((track, trackIndex) => (track.id || `${track.language}-${trackIndex}`) === subtitleId);
        if (index < 0) return;
        ...
        <div className="space-y-3 rounded-lg border border-border/80 bg-muted/20 p-3">
            ...
            <div className="grid gap-3 lg:grid-cols-2">
                <div className="space-y-2 rounded-md border bg-background/70 p-2.5">
                    ...
                    <Select value={selectedQuality?.quality || videoQuality} onValueChange={persistVideoQuality}>
                        <SelectTrigger className="h-9"><SelectValue placeholder={copy.best} /></SelectTrigger>
                        <SelectContent>
                            {qualityOptions.map((option) => {
                                const details = [option.label || option.quality, option.filesize ? formatBytes(option.filesize) : null]
                                    .filter(Boolean).join(' · ');
                                return <SelectItem key={option.quality} value={option.quality}>{details}</SelectItem>;
                            })}
                        </SelectContent>
                    </Select>
                    <Button type="button" className="w-full" onClick={handleVideoDownload}>
                        <Download className="h-4 w-4" />{copy.downloadVideo}
                    </Button>
                    <p className="text-[11px] leading-relaxed text-muted-foreground">{copy.qualityHint}</p>
                </div>

                <div className="space-y-2 rounded-md border bg-background/70 p-2.5">
                    <div className="flex items-center gap-1.5 text-xs font-medium">
                        <Music className="h-3.5 w-3.5" />{copy.audioQuality}
                    </div>
                    ...
                    <Button type="button" variant="secondary" className="w-full" onClick={handleAudioDownload}>
                        <Download className="h-4 w-4" />{copy.downloadAudio}
                    </Button>
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                    <Button type="button" variant="outline" size="sm" disabled={!result.cover} onClick={handleCoverDownload}>
                        <Image className="h-4 w-4" />{copy.cover}
                    </Button>

                    {subtitles.length > 0 ? (
                        <div className="flex min-w-0 gap-1">
                            <Select value={subtitleId} onValueChange={setSubtitleId}>
                                <SelectTrigger className="h-8 min-w-0 flex-1 text-xs"><SelectValue placeholder={copy.subtitles} /></SelectTrigger>
                                <SelectContent>
                                    {subtitles.map((track, index) => {
                                        const value = track.id || `${track.language}-${index}`;
                                        return (
                                            <SelectItem key={value} value={value} disabled={!resolveSubtitleUrl(track)}>
                            ...
                            <Button type="button" variant="outline" size="icon" className="h-8 w-8 shrink-0" onClick={handleSubtitleDownload}>
                                <Captions className="h-4 w-4" /><span className="sr-only">{copy.subtitles}</span>
                            </Button>
                        </div>
                    ) : (
                        <Button type="button" variant="outline" size="sm" disabled title={copy.noSubtitles}>
                            <Captions className="h-4 w-4" />{copy.subtitles}
                        </Button>
                    )}

                    <Button type="button" variant="outline" size="sm" onClick={handleMetadataDownload}>
                        <FileJson className="h-4 w-4" />{copy.metadata}
                    </Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => void handleCopySource()}>
                        <Clipboard className="h-4 w-4" />{copy.copySource}
                    </Button>
                </div>
                {subtitles.length === 0 && <p className="text-[11px] text-muted-foreground">{copy.noSubtitles}</p>}
            </div>
        </div>
    );
}
