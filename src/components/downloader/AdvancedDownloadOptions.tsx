...
const COPY: Record<string, Copy> = {
    zh: {
        title: '高级下载选项',
        quality: '视频清晰度',
        qualityHint: '优先按所选清晰度重新向解析后端请求，不再直接使用解析结果中的低清临时流。',
        parserFormats: '解析器返回的可用格式',
        genericPresets: '通用清晰度预设；平台不支持时由后端选择最接近/最佳可用画质',
    ...
    'zh-tw': {
        title: '進階下載選項',
        quality: '影片畫質',
        qualityHint: '優先依所選畫質重新向解析後端請求，不再直接使用解析結果中的低畫質臨時串流。',
        parserFormats: '解析器回傳的可用格式',
        genericPresets: '通用畫質預設；平台不支援時由後端選擇最接近／最佳可用畫質',
    ...
    en: {
        ...
        qualityHint: 'Requests a fresh stream for the selected quality instead of reusing a low-resolution temporary stream.',
        parserFormats: 'Formats reported by the parser',
        genericPresets: 'Generic quality presets; the backend may select the nearest/best available quality when needed',
        ...
        noSubtitles: 'No subtitle tracks were returned by the parser',
    ...
    ja: {
        title: '詳細ダウンロード設定',
        quality: '動画画質',
        qualityHint: '低画質の一時ストリームを再利用せず、選択した画質で解析バックエンドへ再リクエストします。',
        parserFormats: '解析結果の利用可能フォーマット',
        genericPresets: '共通画質プリセット。未対応の場合はバックエンドが最も近い／最良の画質を選択します',
    ...
    es: {
        ...
        qualityHint: 'Solicita un flujo nuevo con la calidad elegida en lugar de reutilizar un flujo temporal de baja resolución.',
        parserFormats: 'Formatos disponibles informados por el analizador',
        genericPresets: 'Preajustes genéricos; el servidor puede elegir la calidad más cercana o la mejor disponible',
        ...
        noSubtitles: 'El analizador no devolvió pistas de subtítulos',
    ...
    ru: {
        ...
        qualityHint: 'Запрашивает новый поток выбранного качества вместо повторного использования временного потока низкого разрешения.',
        parserFormats: 'Форматы, доступные по данным парсера',
        genericPresets: 'Универсальные пресеты; сервер может выбрать ближайшее или лучшее доступное качество',
        ...
        noSubtitles: 'Парсер не вернул дорожки субтитров',
...
export function AdvancedDownloadOptions({ result }: { result: ResultData }) {
    ...
    const qualityOptions = useMemo(
        () => normalizeQualityOptions(result.qualityOptions),
        [result.qualityOptions]
    );
    const parserProvidedFormats = Boolean(result.qualityOptions?.length);
    ...
    useEffect(() => {
        if (subtitles.length > 0 && !subtitleId) {
            setSubtitleId(subtitles[0].id || `${subtitles[0].language}-0`);
        }
    ...
    const selectedQuality = qualityOptions.find((option) => option.quality === videoQuality)
        || qualityOptions[0];

    const handleVideoDownload = () => {
        if (!sourceUrl || !selectedQuality) return;

        const url = selectedQuality.downloadUrl
            || buildSourceMediaDownloadUrl({
                sourceUrl,
                type: 'video',
        ...
        const qualityLabel = selectedQuality.height
            ? `${selectedQuality.height}p`
            : selectedQuality.quality === 'best'
                ? 'best'
                : selectedQuality.quality;
        const extension = selectedQuality.ext || 'mp4';
        downloadFile(url, `${safeTitle}-${qualityLabel}.${extension}`);
    };
    ...
    const handleAudioDownload = () => {
        ...
        downloadFile(url, `${safeTitle}-audio.${audioQuality === 'best' ? 'm4a' : 'm4a'}`);
    };
    ...
    const handleMetadataDownload = () => {
        saveJsonFile(`${safeTitle}-info.json`, createMediaMetadata(result));
    };
                    ...
                    <Select value={selectedQuality?.quality || videoQuality} onValueChange={persistVideoQuality}>
                        ...
                        <SelectContent>
                            {qualityOptions.map((option) => {
                                ...
                                return (
                                    <SelectItem key={`${option.quality}-${option.formatId ?? 'preset'}`} value={option.quality}>
                                        {details}
                                    </SelectItem>
                                );
                            })}
