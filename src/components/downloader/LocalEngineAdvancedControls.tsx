...
const COPY: Record<string, Copy> = {
  zh: {
    title: '高级本机下载',
    summary: '片段、章节、字幕/音轨、Bilibili 弹幕、SponsorBlock、aria2c',
    segment: '视频片段',
    start: '开始',
    end: '结束',
...
function isNativeDanmakuTrack(track: SubtitleTrack): boolean {
  return track.language?.trim().toLowerCase() === 'danmaku'
    && track.format?.trim().toLowerCase() === 'xml';
}
...
export function LocalEngineAdvancedControls({
  ...
  const detectedSubtitles = useMemo(() => {
    ...
    for (const track of subtitles) {
      if (isNativeDanmakuTrack(track)) continue;
                ...
                <div className="mt-1.5 flex flex-wrap gap-1" aria-label={copy.detected}>
                  {detectedSubtitles.map((track, index) => {
                    ...
                    return (
                      ...
                      >
                        {track.language} · {track.isAutoGenerated ? copy.manualShort : copy.autoShort}
                      </button>
                    );
                  })}
                </div>
