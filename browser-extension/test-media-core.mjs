...
assert.deepEqual([...manifest.permissions].sort(), ['downloads', 'storage', 'webRequest'])
...
assert(backgroundSource.includes('galaxy:get-settings'))
assert(backgroundSource.includes('galaxy:save-settings'))
...
assert(browserSource.includes('&preview=1'))
assert(browserSource.includes('桌面预览'))
