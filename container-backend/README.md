...
- Xiaohongshu / RedNote: anonymous first; `No video formats found` may trigger a cookie retry because anonymous pages can omit media metadata. An optional regional route can be isolated with `YTDLP_XHS_PROXY`.
...
- `YTDLP_XHS_PROXY` — optional Xiaohongshu-only regional egress.
...
## Xiaohongshu resolver direction

The first-party path remains yt-dlp anonymous -> cookie retry. The next fallback is intentionally designed as a separate XHS-specific resolver rather than embedding Chromium into this media image. Projects such as `xhs-downloader` separate HTTP and browser providers and can switch between HTTP-first and browser-first strategies; Galaxy will follow the same architectural boundary through an optional resolver integration rather than copying provider code into the core backend.
