...
def run_test():
    install_desktop_marketplace(FakeEngine)
    assert getattr(FakeWindow,'_galaxy_desktop_marketplace_installed',False)
    assert registered_after_build_ui_hooks(FakeWindow).count('desktop-plugin-marketplace')==1
    install_desktop_marketplace(FakeEngine)
    assert registered_after_build_ui_hooks(FakeWindow).count('desktop-plugin-marketplace')==1
    run_desktop_marketplace_self_test()
if __name__=='__main__': run_test(); print('Desktop Marketplace self-test passed')
