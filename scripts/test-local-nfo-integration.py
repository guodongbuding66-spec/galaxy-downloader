...
class NfoIntegrationUnitTests(unittest.TestCase):
    ...
    def test_policy_embedded_success_promotes_nfo_and_cleans_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            engine = self._fake_engine(root, malformed=False)
            ...
            try:
                ...
                self.assertTrue(engine.job_from_payload({"sourceUrl": "https://example.com/v", "includeNfo": True}).include_nfo)
    ...
    def test_nfo_failure_never_changes_completed_media_state(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            engine = self._fake_engine(root, malformed=True)
            original_builder = external_ytdlp.build_external_command
            try:
    ...
    @staticmethod
    def _fake_engine(root: Path, *, malformed: bool):
