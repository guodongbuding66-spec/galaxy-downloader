...
def _item_from_history(engine_module, item: object) -> dict[str, Any] | None:
    ...
    try:
        duration = max(0.0, min(float(item.get("durationSeconds") or 0), 30 * 24 * 3600))
...
def _upsert(connection: sqlite3.Connection, value: dict[str, Any]) -> None:
    existing = connection.execute("SELECT id FROM media_items WHERE file_path=?", (value["file_path"],)).fetchone()
    if existing is None:
        connection.execute(
            """
...
def _public_row(row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "sourceHost": str(row["source_host"]),
        "videoQuality": str(row["video_quality"]),
        "audioQuality": str(row["audio_quality"]),
        "collectionMode": str(row["collection_mode"]),
...
def resolve_media_item_path(engine_module, item_id: object) -> Path | None:
...
def set_media_item_tags(engine_module, item_id: object, tags: Iterable[object]) -> tuple[str, ...]:
    clean_id = str(item_id or "").strip()
    if not clean_id:
        return ()
    cleaned: list[str] = []
    for raw in tags:
        tag = _safe_text(raw, 60)
...
def run_media_library_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        history = [
            ...
            {"state": "failed", "filePath": str(downloads / "failed.mp4")},
        ]
        ...
        assert resolve_media_item_path(Engine, items[0]["id"]) == video.resolve()
        assert set_media_item_tags(Engine, items[0]["id"], ["course", "course", "demo"]) == ("course", "demo")
        assert search_media_items(Engine, "course")[0]["id"] == items[0]["id"]

        video.unlink()
        sync_media_library(Engine, [])
        summary = media_library_summary(Engine)
        assert summary.available == 0 and summary.missing == 1
        assert resolve_media_item_path(Engine, items[0]["id"]) is None
