...
from subscriptions import ALLOWED_INTERVALS, add_subscription, check_subscription, delete_subscription, load_subscriptions, update_subscription
...
INTERVAL_LABELS = {15: "15 分钟", 30: "30 分钟", 60: "1 小时", 180: "3 小时", 360: "6 小时", 720: "12 小时", 1440: "24 小时"}
...
def _edit_subscription_dialog(parent, engine_module, existing: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    def entry(variable: tk.StringVar) -> tk.Entry:
        widget = tk.Entry(card, textvariable=variable, font=("Segoe UI", 9), bg=ui.BG, fg=ui.TEXT, insertbackground=ui.TEXT, relief="flat", bd=0, highlightthickness=1, highlightbackground=ui.BORDER, highlightcolor=ui.ACCENT)
    ...
    for text, variable in (("启用这个订阅", enabled_var), ("发现新内容后自动加入下载队列", auto_var), ("下载视频时包含音频", include_audio_var)):
        tk.Checkbutton(options, text=text, variable=variable, font=("Segoe UI", 8), bg=ui.PANEL, fg=ui.MUTED, activebackground=ui.PANEL, activeforeground=ui.TEXT, selectcolor=ui.BG, bd=0, highlightthickness=0).pack(anchor="w", pady=2)

    ui._label(card, "自动下载只有在你明确勾选后才会产生后台网络检查；Galaxy 关闭时不会继续轮询。", size=7, color=ui.SUBTLE, wraplength=560, justify="left").pack(anchor="w", pady=(9, 0))
...
def _show_subscriptions(window, engine_module) -> None:
    ...
    ui._label(shell, "频道/播放列表订阅保存在本机。首次检查只建立基线；之后仅处理新条目。自动下载关闭时不会自动联网检查。", size=8, color=ui.MUTED, bg=ui.BG, wraplength=900, justify="left").pack(anchor="w", pady=(4, 12))
    ...
    headings = {"title": ("名称", 190), "source": ("来源", 150), "enabled": ("启用", 55), "auto": ("自动下载", 72), "interval": ("间隔", 70), "last": ("最近检查", 150), "result": ("状态", 250)}
    ...
    def refresh() -> None:
        ...
        for index, item in enumerate(items):
            ...
            host = str(item.get("sourceUrl") or "").split("//", 1)[-1].split("/", 1)[0]
            ...
            tree.insert("", "end", iid=iid, values=(item.get("title") or "未命名订阅", host or "—", "是" if item.get("enabled") else "否", "是" if item.get("autoDownload") else "否", INTERVAL_LABELS.get(interval, f"{interval} 分钟"), _format_time(item.get("lastCheckedAt")), state_text))
        if selected_id:
        ...
        def worker() -> None:
            try:
                if auto_download:
                    ...
                    message = "首次检查完成：已建立当前内容基线。" if result.baseline else f"检查完成：发现 {len(result.new_entries)} 项新内容；自动下载未开启，因此仅标记为已查看。"
                error = ""
            except Exception as exc:
                message = ""
                error = str(exc)
...
def install_desktop_subscriptions(engine_module):
    ...
    def after_build_ui(window) -> None:
        ...
        window._subscriptions_button = ui.ActionButton(queue_head, text="订阅", command=lambda: show_desktop_presenter(window, "subscriptions"), kind="ghost", compact=True)
