...
def _with_learning_assets(body: bytes, file_name: str) -> bytes:
    ...
    try:
        ...
        return body
    for style_tag in (_LEARNING_STYLE_TAG, _ATTACHMENT_STYLE_TAG, _NAVIGATION_STYLE_TAG):
        if style_tag not in html:
            html = html.replace("</head>", f"  {style_tag}\n</head>", 1)
    for script_tag in (_LEARNING_SCRIPT_TAG, _ATTACHMENT_SCRIPT_TAG, _NAVIGATION_SCRIPT_TAG):
