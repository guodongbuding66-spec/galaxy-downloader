...
def install_nfo_sidecar_policy(engine_module):
    ...
    def run_job(window) -> None:
        ...
        try:
            ...
            remaining, truncated = _discover_info_json_files(temp_dir)
            if status == "completed":
                if remaining:
                    result = _promote_nfo_sidecars(temp_dir, Path(engine_module.default_download_dir()))
                    _record_result(window, result)
                else:
                    current = str(window.bridge_status().get("nfoSidecarStatus") or "").lower()
                    if current not in {"saved", "partial", "failed"}:
                        _record_result(window, NfoPromotionResult((), (), truncated))
            elif remaining:
                window._update_bridge(
                ...
                window._update_bridge(
                    nfoSidecarStatus=status,
                    nfoSidecarWarning=(
                        "Media did not complete; no NFO sidecar was promoted."
                    ),
                    nfoSidecarSavedCount=0,
                )
        finally:
