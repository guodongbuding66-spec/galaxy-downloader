...
def run_auto_check(engine_module, subscription_id: str, submit: Callable[[dict[str, Any]], object]) -> AutoCheckResult:
    ...
    subscription = next((item for item in load_subscriptions(engine_module) if str(item.get("id")) == subscription_id), None)
...
def run_subscription_scheduler_self_test() -> None:
    ...
    assert not subscription_due(
        {"enabled": True, "autoDownload": True, "intervalMinutes": 60, "lastCheckedAt": "2026-09-02T23:30:00Z"},
        now=now,
    )
    assert subscription_due(
        {"enabled": True, "autoDownload": True, "intervalMinutes": 60, "lastCheckedAt": "2026-09-02T22:59:00Z"},
        now=now,
    )
